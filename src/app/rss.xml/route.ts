...
function renderTweetForRss(tweetId: string): string {
  ...
  if (!tweet?.text || !tweet.created_at || !tweet.author?.name || !tweet.author?.username) {
  ...
  const authorUsername = author.username ?? siteConfig.author.twitterUsername;
