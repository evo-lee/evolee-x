...
- `src/app/api/chat/route.ts`
  ...
  - 关键词提取提示词强化为“主题实体词优先”，显式规避数量词/功能词污染
  - 搜索 0 命中时自动回退到本地 query 重新检索
  - 追问出现新实体词时，禁止复用旧检索上下文并强制重检索
  - 对宽泛主题词（如“马拉松”“日本”）自动扩大召回窗口，并用相对分数阈值过滤弱相关结果
  - 锚点词采用“query term 在候选结果中的覆盖率”自动选取区分度最高的词，不依赖主题词硬编码
  - 默认召回上限提升：博客 `12/24`（普通/宽查询），X `8/18`
  - 首轮问题默认也可触发关键词模型（可通过 env 关闭），降低对本地停用词表的依赖
- `src/lib/chat-prompts/runtime-context.ts` 与 `legacy-v1.ts`
  - 在“相关文章/相关动态”区块中显式注入“检索命中总量 + 当前展示数量 + 省略数量”
  - 数量类问答（如“写过几篇”“去过几次”）可直接依据检索总量回答，减少只看前几条导致的低估
